...
async def github_callback(req, resp):
     ...
     cookie       = """<br>run this to save your login cookie:<br><textarea style="margin: 0px; width: 569px; height: 85px;">echo "oneliner.sh FALSE / FALSE 0 session """ + session_id + """" | sed -e 's/\\s/\\t/g' > ~/.oneliner.sh.cookie.txt
</textarea><br>
---<br>
and then to verify your session, run:<br><textarea style="margin: 0px; width: 569px;height: 55px;">curl -L -b ~/.oneliner.sh.cookie.txt oneliner.sh/me
</textarea>
"""
